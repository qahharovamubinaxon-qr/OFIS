...
_PASSPORT = _COMMON + (
    ...
    '(Amal qilish muddati), authority (Kim tomonidan berilgan). IGNORE the '
    'two «<<<» lines at the very bottom entirely — that strip is not to be '
    'read, quoted or used for anything. EVERY value comes from the printed '
    'rows above it.\n'
    ...
    '"patronymic_latin" — KAKHOROV, ABBOSBEK, ANVAROVICH. Copy that row as '
    'it stands: do not translate it, do not simplify it, do not drop a '
    'letter (KAKHOROV is not KAKOROV), and keep the ʻ of Oʻ and Gʻ where the '
    'page prints one. The program transliterates those Latin rows into '
    'Russian itself, by the office\'s own rules, so an exact copy matters '
    'more than your Cyrillic. If a row is genuinely not printed in Latin, '
    'leave that key "" — never invent one. '
