...
_SYSTEM = (
    ...
    "добавляет программа. КАЖДЫЙ абзац пиши ОДНОЙ длинной строкой — не "
    "переноси строки внутри абзаца и не выравнивай текст пробелами. Верни "
    "ТОЛЬКО текст документа, без пояснений и без markdown."
    ...
    def _to_docx(text: str, out: Path) -> Path:
        ...
        for role, content in _classify(text.strip().splitlines()):
            if role == "title":
                ...
                if not content.lower().startswith(("подпись", "зарегистрировано",
                                                   "уплачено")):
